...
class welcome_event(commands.Cog):
    def __init__(self, client):
        ...
        self.client = pymongo.MongoClient("mongodb+srv://workforkami:DqdgZf7yCSUS9V3c@Luka.qvswi7j.mongodb.net/welcome?retryWrites=true&w=majority")
        self.db = self.client.welcome.welcomedata

    @commands.Cog.listener()
    async def on_member_join(self, user):
        try:
            guild = user.guild
            data = self.db.find_one({"guild": guild.id})
